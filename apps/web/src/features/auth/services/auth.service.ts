...
// without it the link would point at the API, which renders nothing.
...
// The account has a second factor: the password was right, and a code is needed
// before the session authenticates anything. The form switches to the code step.
export class MfaRequiredError extends Error {}

function throwIfMfaRequired(result: { data: { status?: string } | null }): void {
  if (result.data?.status === 'mfa_required') throw new MfaRequiredError('mfa_required');
}
...
// The second step: a TOTP or recovery code against the half-open session.
export async function verifyMfaCode(code: string): Promise<void> {
  const result = await signIn.mfa({ code: code.trim() });
  if (result.error) throw new Error(result.error.message ?? '');
}

// Redeems the token from an emailed sign-in link.
export async function signInWithMagicLink(token: string): Promise<void> {
  const result = await signIn.magicLinkVerify({ token });
  if (result.error) throw new Error(result.error.message ?? '');
  throwIfMfaRequired(result);
}
...
// Sends the password reset link. The address is not confirmed to exist: the api
...
// the new password is set: the api keeps the query it is given and adds its own
// `token` to it.
...
// instead of returning a result: the browser is sent to Google, and Google comes
...
// The sign-in link points here with ?magic=<token>, so the token never reaches
// the api origin in a URL bar. The redirect is where to go once signed in.
export function magicLinkToken(params: URLSearchParams): string | null {
